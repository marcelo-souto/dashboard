...
>(
  (
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      if (labelFormatter) {
        return (
          <div className={cn("font-semibold font-heading", labelClassName)}>
      ...
      return <div className={cn("font-semibold font-heading", labelClassName)}>{value}</div>
    }, [
      label,
      labelFormatter,
      payload,
        ...
        {payload.map((item) => {
          ...
          return (
            <div
              key={item.value}
              className={cn(
                "flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-zinc-500 dark:[&>svg]:text-zinc-400 font-heading"
              )}
            >
